...
function setUpBeacons() {
	...
	if (global.USB_DEVICES == 0) {
		showNotification({
			title: 'No beacon devices found.',
			body: 'If you wish to use beacon with a USB busylight, please connect a compatible device and rescan for USB devices from the menu.',
			showNotification: true,
		})
	} else {
		showNotification({
			title: `${global.USB_DEVICES} device${global.USB_DEVICES > 1 ? 's' : ''} found.`,
			body: 'You can now use the API to control your beacon devices.',
			showNotification: true,
		})
	}
}
